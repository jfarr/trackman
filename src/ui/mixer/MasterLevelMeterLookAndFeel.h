...
class MasterLevelMeterLookAndFeel : public LevelMeterLookAndFeel {
  public:
        ...
        if (meterType & LevelMeter::Minimal) {
            ...
            for (int channel = 0; channel < numChannels; ++channel) {
                ...
                if (channel < numChannels - 1) {
                    ...
                    Rectangle<float> ticks = getMeterTickmarksBounds(meter, meterType).withWidth(width * 1.5);
                    if (!ticks.isEmpty())
    ...
    void drawTickMarks(Graphics &g, LevelMeter::MeterFlags meterType, Rectangle<float> bounds) override {
        ...
        for (int i = 0; i < 11; ++i) {
            g.drawHorizontalLine(roundToInt(bounds.getY() + i * h + 1), bounds.getX() + 4, bounds.getRight());
        }
